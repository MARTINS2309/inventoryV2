  ...
  const handleCreate = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        handleAlert(
          `${
            url2 ? "Stock Event" : event.target.name.value
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/${url}/${item.id}`,
        {
          method: "PUT",
          body: JSON.stringify(body),
          headers: {
  ...
  const handleDelete = async (event) => {
    ...
    try {
      ...
      if (response.status === 200) {
        handleClose();
        handleAlert(
          `${url2 ? "Stock Event" : item.name} deleted successfully`,
          "success"
        );
      ...
      handleAlert(
        `Error deleting ${url2 ? "Stock Event" : item.name} error: ${error}`,
        "danger"
      );
      console.log("error", error);
    }
    ...
    <div className={`CRUDTable_${url}`}>
      <div className="Table">
        <TableT
          columns={columns}
          data={
            prod_id ? items.filter((it) => it.product_id === prod_id) : items
          }
